...
#ifndef EXAMPLES_ANALYTICAL_APPS_CUDA_LCCDOPT_LCCDOPT_H_
#define EXAMPLES_ANALYTICAL_APPS_CUDA_LCCDOPT_LCCDOPT_H_
#ifdef __CUDACC__
#include <iomanip>
#include <iostream>
 ...
 public:
  ...
  void IncEval(const fragment_t& frag, context_t& ctx,
    ...
    if (ctx.stage == 1) {
      ...
      // WorkSourceRange<vertex_t> ws_in(*iv.begin(), iv.size());
      // ForEach(stream, ws_in, [=] __device__(vertex_t v) mutable {
      //  printf("vertex:%ld has %ld triangle and degree is %ld\n",
      //         d_frag.GetId(v), d_tricnt[v], d_global_degree[v]);
      //});
    }
...
#endif  // __CUDACC__
#endif  // EXAMPLES_ANALYTICAL_APPS_CUDA_LCCDOPT_LCCDOPT_H_
